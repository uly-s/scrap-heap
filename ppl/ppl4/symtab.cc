...
symbol_t * symbol_create (symtab_t * symtab, string p_name, int p_type, int p_size)
{
  symbol_t * ret;
  ...
  ret->datatype = p_type;
  ret->size = p_size;
  symtab->offset += sizeof(int) * (p_size + 1);
...
symbol_t * make_temp (symtab_t * symtab, int dtype)
{
  ...
  symbol_t * ret = symbol_create (symtab, string(tempname), dtype, 1);
